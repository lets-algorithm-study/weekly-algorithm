...
function solution (prices) {
  ...
  for(let i = 0; i < prices.length; i++) {
    let currentPrices = prices.slice(i); // i번째부터 끝까지의 배열
    let stack = [currentPrices[0]]; // 현재 시작하는 가격

    for(let [index,p] of currentPrices.entries()){
      if(stack.length > 0 && stack[0] <= p){
        stack.push(p)
        if(currentPrices.length - 1 == index){ 
